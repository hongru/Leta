!function (name, definition) {
  ...
  else if (this['Leta'] != undefined) {
	this['Leta'].register('.'+name, definition);
  } else {
  ...
  // $D(elements).css('color', function (el) {
  ...
  $D.prototype = {
    ...
    , replaceWith: function(html) {
        return this.each(function (el) {
          el.parentNode.replaceChild($D.create(html)[0], el)
        })
      }
  ...
  function normalize(node) {
    return typeof node == 'string' ? $D.create(node) : isNode(node) ? [node] : node // assume [nodes]
  }
  ...
  $D.setQueryEngine = function (q) {
    query = q;
    delete $D.setQueryEngine
  }

  $D.aug = function (o, target) {
    for (var k in o) {
      o.hasOwnProperty(k) && ((target || $D.prototype)[k] = o[k])
    }
  }

  $D.create = function (node) {
  ...
  $D.isAncestor = 'compareDocumentPosition' in html ?
